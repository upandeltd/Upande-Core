...
class AccessDashboard {
	...
	render_shell() {
		...
		this.page.main.find(".ra-rescan").on("click", () => {
			...
			frappe
				.xcall("upande_core.anomalies.overview", { force: 1 })
				.then(() => this.load());
		});
	}
	...
	async open_user(user) {
		...
		let d;
		try {
			d = await frappe.xcall("upande_core.dashboard.user_detail", { user });
		} catch (error) {
	...
	async open_profile(profile) {
		...
		const d = await frappe.xcall("upande_core.dashboard.profile_detail", { profile });
	...
	async open_module(module) {
		...
		const d = await frappe.xcall("upande_core.dashboard.module_detail", { module });
	...
	async open_log(name) {
		...
		const d = await frappe.xcall("upande_core.dashboard.log_detail", { name });
	...
	async fetch(key, method, args) {
		...
		if (!this.cache[key]) {
			const path = method.includes(".") ? method : `upande_core.dashboard.${method}`;
	...
	async view_anomalies() {
		...
		let data;
		try {
			data = await this.fetch("anomalies", "upande_core.anomalies.overview");
		} catch (error) {
	...
	async open_anomaly(kind) {
		...
		let d;
		try {
			d = await frappe.xcall("upande_core.anomalies.detail", { kind });
		} catch (error) {
			$drawer.find(".ra-drawer__body").html(`<div class="ra-empty">${esc(error.message)}</div>`);
			return;
		}
	...
	async render_needs_mode() {
		...
		let modules;
		try {
			modules = await this.fetch("grant_modules", "upande_core.grant.modules");
		} catch (error) {
	...
	async load_documents(txt = "") {
		...
		let docs;
		try {
			docs = await frappe.xcall("upande_core.grant.documents", {
	...
	async resolve_grant() {
		...
		let verdict;
		try {
			verdict = await frappe.xcall("upande_core.grant.resolve", {
				user,
	...
	async do_grant(profile) {
		...
		frappe.confirm(
			...
			async () => {
				...
				try {
					const result = await frappe.xcall("upande_core.grant.apply", {
						user,
	...
	async do_add() {
		...
		let plan;
		try {
			plan = await frappe.xcall("upande_core.compose.preview", {
				user,
	...
	async commit_add(plan, user) {
		...
		try {
			const result = await frappe.xcall("upande_core.compose.apply", {
				user,
				requirements: this.grant.needs,
	...
	async record_grant_as_request(user) {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: async ({ reason }) => {
				dialog.hide();
				try {
					const out = await frappe.xcall("upande_core.requests.submit_request", {
						user,
						requirements: this.grant.needs,
						reason,
					});
	...
	async render_profile_mode() {
		...
		let profiles;
		try {
			profiles = await this.fetch("grantable", "upande_core.api.get_grantable_profiles");
		} catch (error) {
	...
	async on_pick_user() {
		...
		let cov;
		try {
			cov = await frappe.xcall("upande_core.grant.coverage", { user });
		} catch (error) {
	...
	async preview(profile) {
		...
		let diff;
		try {
			diff = await frappe.xcall("upande_core.api.preview_assignment", {
				user,
				role_profile: profile,
			});
		} catch (error) {
	...
	async view_queue() {
		...
		const rows = await this.fetch("queue", "upande_core.requests.queue", { limit: 100 });
		...
		const draw = () => {
			...
			$body.find(".ra-recheck").on("click", async (event) => {
				...
				try {
					const v = await frappe.xcall("upande_core.requests.recheck", { request: name });
					frappe.show_alert({
			...
			$body.find(".ra-grant").on("click", (event) => {
				...
				frappe.confirm(
					...
					async () => {
						try {
							const result = await frappe.xcall("upande_core.requests.fulfil", {
								request: name,
							});
							frappe.show_alert({
								message: __("{0} now holds {1}", [
			...
			$body.find(".ra-refuse").on("click", (event) => {
				...
				const dialog = new frappe.ui.Dialog({
					...
					primary_action: async (values) => {
						try {
							await frappe.xcall("upande_core.requests.refuse", {
								request: name,
								note: values.note,
							});
							dialog.hide();
	...
	async search_docs(txt) {
		...
		const [docs, bundles] = await Promise.all([
			frappe.xcall("upande_core.requests.search_documents", { txt, limit: 18 }),
			frappe.xcall("upande_core.requests.search_bundles", { txt, limit: 5 }),
		]);
	...
	async add_document(doctype) {
		...
		const rights = await frappe.xcall("upande_core.requests.rights_for", { doctype });
	...
	async add_bundle(bundle) {
		const rows = await frappe.xcall("upande_core.requests.expand_bundle", { bundle });
		for (const row of rows) {
			...
			const rights = await frappe.xcall("upande_core.requests.rights_for", {
	...
	async resolve_request() {
		...
		let v;
		try {
			v = await frappe.xcall("upande_core.requests.resolve", { user, requirements });
		} catch (error) {
	...
	async record_request(and_assign) {
		...
		try {
			const result = await frappe.xcall("upande_core.requests.submit_request", {
				user,
			...
			if (and_assign && result.profile) {
				const outcome = await frappe.xcall("upande_core.requests.fulfil", {
	...
	async view_map() {
		...
		const draw = () => {
			...
			this.$main.find(".ra-mapactive").on("change", async (event) => {
				...
				try {
					await frappe.xcall("upande_core.dashboard.map_update", {
						name,
						field: "is_active",
						value,
					});
		...
		this.$main.find(".ra-sweeprun").on("click", () => {
			...
			frappe.confirm(
				...
				async () => {
					...
					$button.html(`<span class="ra-spinner"></span> ${__("Assigning…")}`);
					try {
						const result = await frappe.xcall("upande_core.dashboard.sweep_apply", { users });
						frappe.msgprint({
	...
	async view_admin() {
		...
		const [delegates, policy] = await Promise.all([
			frappe.xcall("upande_core.dashboard.delegate_list"),
			frappe.xcall("upande_core.dashboard.settings_read"),
		]);
		...
		this.$main.find(".ra-deltoggle").on("change", async (event) => {
			...
			try {
				await frappe.xcall("upande_core.dashboard.delegate_toggle", { user, enabled });
				frappe.show_alert({
		...
		this.$main.find(".ra-savepolicy").on("click", async (event) => {
			const $button = $(event.currentTarget).prop("disabled", true);
			try {
				await frappe.xcall("upande_core.dashboard.settings_write", {
					values: {
	...
	new_delegate() {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: async (values) => {
				try {
					const result = await frappe.xcall("upande_core.dashboard.delegate_save", {
						user: values.user,
