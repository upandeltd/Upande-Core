 ...
 * upande_core.api instead. This page is a thin client over those four methods:
...
class UserAccessConsole {
	...
	async load_profiles() {
		try {
			const [users, profiles] = await Promise.all([
				frappe.xcall("upande_core.api.get_manageable_users", { limit: 500 }),
				frappe.xcall("upande_core.api.get_grantable_profiles"),
			]);
	...
	async on_user_change() {
		...
		if (!row) {
			...
			const fetched = await frappe.xcall("upande_core.api.get_manageable_users", {
				search: user,
				limit: 1,
			});
			row = (fetched || [])[0];
	...
	async preview(role_profile) {
		...
		const diff = await frappe.xcall("upande_core.api.preview_assignment", {
