...
class MyAccess {
	...
	async load() {
		const [me, requests] = await Promise.all([
			frappe.xcall("upande_core.requests.my_access"),
			frappe.xcall("upande_core.requests.my_requests", { limit: 15 }),
		]);
		this.me = me;
	...
	async search(txt) {
		...
		const [docs, bundles] = await Promise.all([
			frappe.xcall("upande_core.requests.search_documents", { txt, limit: 15 }),
			frappe.xcall("upande_core.requests.search_bundles", { txt, limit: 4 }),
		]);
	...
	async check() {
		...
		let v;
		try {
			v = await frappe.xcall("upande_core.requests.check_for_me", { requirements });
		} catch (error) {
	...
	async send() {
		...
		try {
			const result = await frappe.xcall("upande_core.requests.request_for_me", {
			...
			this.draw_basket();
			this.draw_requests(
				await frappe.xcall("upande_core.requests.my_requests", { limit: 15 })
			);
		} catch (error) {
