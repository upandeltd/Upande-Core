...
from upande_core import capability, dashboard
...
def _attribute(rows: list[dict]) -> list[dict]:
	...
	for row in rows:
		...
		if match:
			# Names the mechanism, not the app: the UI chip reads "via Role
			# Advisor", and the tag survived the fold into upande_core unchanged
			# so the front end and this stay in step. Do not rename to the app.
